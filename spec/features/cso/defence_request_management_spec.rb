...
RSpec.feature 'custody suite officers managing defence requests' do
  ...
  context 'creating a new request' do
    scenario 'can not see a DSCC field on the defence request form' do
      visit root_path
    ...
    scenario 'can not see the solicitor time of arrival field on the defence request form' do
      visit root_path
      click_link 'New Defence Request'
    ...
    scenario 'can fill in form manually with all relevant details for own solicitor', js: true do
      visit root_path
      click_link 'New Defence Request'
    ...
    scenario 'can choose own solicitor and pick a solicitor using a from search box from the form', js: true do
    ...
    scenario 'can perform multiple own solicitor searches', js: true do
      stub_solicitor_search_for_bob_smith
    ...
    scenario "is shown a message when no results are found for their search", js: true do
    ...
    scenario "can clear the solicitor search results with a close button", js: true do
      stub_solicitor_search_for_bob_smith
      visit root_path
      click_link 'New Defence Request'
      choose 'Own'
    ...
    scenario "can clear the solicitor search results by pressing the ESC key", js: true do
      stub_solicitor_search_for_bob_smith
      visit root_path
      click_link 'New Defence Request'
      choose 'Own'
    ...
    scenario "has the solicitor search results cleared when toggling between 'duty' or 'own' solicitor", js: true do
      stub_solicitor_search_for_bob_smith
  ...
  context 'with requests they are assigned to' do
    ...
    scenario 'can close the request from the dashboard' do
      visit root_path
    ...
    scenario 'can close the request from the edit page' do
      visit root_path
      within "#defence_request_#{dr_created.id}" do
        click_link 'Edit'
      end
    ...
    context "with requests that have not been opened yet" do
      scenario 'can edit all relevant details of the request' do
        visit root_path
        within "#defence_request_#{dr_created.id}" do
          click_link 'Edit'
        end
    ...
    context "with accepted requests" do
      ...
      scenario 'can not edit the expected arrival time from request show page' do
